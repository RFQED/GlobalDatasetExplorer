...
with colph:
    ...
    fig = px.scatter_mapbox(df, lat='latitude', lon='longitude', color='ph', zoom=5, center={"lat":mid_lat, "lon":mid_lon})
    fig2 = px.scatter_mapbox(df_chosen_point, lat='lat', lon='lon', size='size', opacity=0.9, zoom=5, center={"lat":mid_lat, "lon":mid_lon})
...
with colbulkdensity:
    ...
    fig = px.scatter_mapbox(df, lat='latitude', lon='longitude', color='bulk_den', zoom=5, center={"lat":mid_lat, "lon":mid_lon})
    fig2 = px.scatter_mapbox(df_chosen_point, lat='lat', lon='lon', size='size', opacity=0.9, zoom=5, center={"lat":mid_lat, "lon":mid_lon})
    fig.add_trace(fig2.data[0])
    fig.update_layout( margin={"r":0,"t":0,"l":0,"b":0},
                            mapbox = { 'style': "mapbox://styles/rfqed/ckx0prtk02gmq15mty3tlmhpu"},
